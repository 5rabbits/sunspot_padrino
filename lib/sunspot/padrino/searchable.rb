    ...
    module Searchable
      ...
      module ClassMethods
        ...
        def solr_benchmark(batch_size, counter,  &block)
          start = Time.now
          Padrino.logger.info("[#{Time.now}] Start Indexing")
          yield
          elapsed = Time.now-start
          Padrino.logger.info("[#{Time.now}] Completed Indexing. Rows indexed #{counter * batch_size}. Rows/sec: #{batch_size/elapsed.to_f} (Elapsed: #{elapsed} sec.)")
        end
